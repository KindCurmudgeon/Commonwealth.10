    ...
    public static async Task GetPortfolioAsync(User user, BlobService blobService, UserResponse response)
    {
        ...
        foreach (NationIdentity identity in user.NationIdentities ?? [])
        {
            ...
            {  
                ...
                NationIdentity? match = user.NationIdentities?.Find(i=> i.GameName == identity.GameName && i.NationCode > 0);
                if (match is not null) continue;    
            }
            else myNation = nations.Find(n => n.Identity.NationCode == identity.NationCode);
            summaries.Add(game.CreateGameSummaryDTO(myNation, waitingCount, user));
        }
