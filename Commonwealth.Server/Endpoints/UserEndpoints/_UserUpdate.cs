...
{
    public static async Task Update(UserDTO userDTO, User user, BlobService blobService, UserResponse response)
    {
        {
            ...
            async Task ParseFriends()
            {
                ...
                {
                    try {
                        User userFriend = await User.RetrieveAsync(friend,blobService);
