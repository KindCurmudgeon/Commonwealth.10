...
{
    public static GameStatus Create(GameDTO dto, PlayerProfile creator, EconParms econParms, GeographyFile geogParmsFile, InitParms initParms)
    {
     ...
     //   IdentifyAllowedVillages();
        ...
        void InitializeMarket()
        {
            ...
            foreach (MarketInit init in initParms.InitialMarketGoods)
            {
                ...
                status.MarketDatas.Add(data);

            }
        }
        void CreateWorldNews()
        {
            status.AddFoodStatusNews(FoodStatus.RATIONING);
            status.AddFoodStatusNews(FoodStatus.FAMINE);
        }
        void CreateGameNews()
        {
            status.GameNews?.AddTextEntry($"Game '{status.Name}' created by '{status.CreatorName}'");
            status.GameNews?.AddTextEntry($"Created: {status.CreationDate: yyyy.MM.dd HH:mm} GMT.");
        }
    }
