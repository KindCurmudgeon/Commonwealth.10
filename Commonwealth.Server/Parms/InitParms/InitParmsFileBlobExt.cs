...
public partial class InitParmsFile : IBlobObject
{
    ...
    public static async Task<InitParmsFile?> RetrieveAsync(ParmFileInfo? info, BlobService blobService)
    {
        if (info is null) return null;
        return await blobService.RetrieveIfExistAsync<InitParmsFile>(BlobPath(info));
    }
}